...
CREATE TABLE IF NOT EXISTS users (
  ...
  role TEXT NOT NULL DEFAULT 'user',  -- 'user', 'admin', 'support' (chat support agent)
  display_id TEXT,
...
-- Chat support agents (role = 'support') are listed on the
-- Admin -> Support Agents page, so that lookup gets its own index.
...
CREATE TABLE IF NOT EXISTS support_messages (
  ...
  sender_type TEXT NOT NULL,          -- user, admin (admins and support agents both send as 'admin')
...
-- Per-agent reply counts on the Admin -> Support Agents page
CREATE INDEX IF NOT EXISTS idx_support_messages_sender ON support_messages(sender_id, sender_type);
