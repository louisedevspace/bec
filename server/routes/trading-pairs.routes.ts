...
export default function registerTradingPairsRoutes(app: Express) {
  ...
  app.post("/api/admin/trading-pairs", requireAuth, requireAdmin, async (req, res) => {
    try {
      const { symbol, baseAsset, quoteAsset, isEnabled, minTradeAmount, maxTradeAmount, tradingFee, tradingFeeUnit, sortOrder, pairType, customApiUrl } = req.body;
      ...
      const { data, error } = await supabaseAdmin
        ...
        .insert({
          ...
          ...(customApiUrl !== undefined && { custom_api_url: customApiUrl || null }),
        })
        .select()
        .single();
  ...
  app.put("/api/admin/trading-pairs/:id", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      const { isEnabled, minTradeAmount, maxTradeAmount, tradingFee, tradingFeeUnit, sortOrder, pairType, customApiUrl } = req.body;
      ...
      if (customApiUrl !== undefined) updateData.custom_api_url = customApiUrl || null;
