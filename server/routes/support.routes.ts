...
import { requireAuth, requireSupportStaff, supabaseAdmin } from "./middleware";
...
export default function registerSupportRoutes(app: Express) {
  ...
  app.post("/api/support/conversation", requireAuth, async (req, res) => {
    try {
      ...
      // OS push to every admin and support agent (best-effort, non-blocking)
      void notifyStaffOfSupportMessage({
        senderId: userId,
        conversationId: conversation.id,
        subject,
        message,
        senderLabel: req.user?.email?.split("@")[0],
        isNewTicket: true,
      });
  ...
  app.get("/api/admin/support/conversations", requireAuth, requireSupportStaff, async (req, res) => {
    try {
      const { data: conversations, error } = await supabaseAdmin
        .from("support_conversations")
        .select(`
  ...
  app.post("/api/admin/support/messages", requireAuth, requireSupportStaff, async (req, res) => {
    try {
      ...
      // OS push to the ticket owner (best-effort, non-blocking)
  ...
  app.post("/api/admin/support/upload-image", requireAuth, requireSupportStaff, upload.single("file"), async (req, res) => {
    try {
      const file = req.file;
      if (!file) return res.status(400).json({ message: "No image file provided" });
  ...
  app.get("/api/admin/support/stats", requireAuth, requireSupportStaff, async (req, res) => {
    try {
  ...
  app.put("/api/admin/support/messages/:messageId/read", requireAuth, requireSupportStaff, async (req, res) => {
    try {
      const { messageId } = req.params;
  ...
  app.put("/api/admin/support/conversations/:id/status", requireAuth, requireSupportStaff, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  app.put("/api/admin/support/conversations/:id/priority", requireAuth, requireSupportStaff, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  app.post("/api/admin/support/conversations/bulk-status", requireAuth, requireSupportStaff, async (req, res) => {
    try {
  ...
  app.put("/api/admin/support/conversations/:id/assign", requireAuth, requireSupportStaff, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  app.put("/api/admin/support/conversations/:id/category", requireAuth, requireSupportStaff, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  app.post("/api/admin/support/conversations/:id/escalate", requireAuth, requireSupportStaff, async (req, res) => {
    try {
      const { id } = req.params;
      const adminId = req.user.id;
  ...
  app.put("/api/admin/support/conversations/:id/bulk-read", requireAuth, requireSupportStaff, async (req, res) => {
    try {
      const { id } = req.params;
