...
export default function AdminDashboard() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "pending_approval": case "pending": return "bg-warning/10 text-warning";
      case "approved": case "executed": case "filled": case "resolved": case "closed": return "bg-success/10 text-success";
      case "rejected": return "bg-danger/10 text-danger";
      case "open": case "in_progress": return "bg-info/10 text-info";
  ...
  const getActivityIcon = (type: string) => {
    switch (type) {
      case 'deposit': return <ArrowDownRight size={14} className="text-success" />;
      case 'withdrawal': return <ArrowUpRight size={14} className="text-danger" />;
      case 'trade': return <BarChart3 size={14} className="text-info" />;
      case 'support': return <MessageSquare size={14} className="text-primary" />;
    ...
    if (!filtered.length) {
      return (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground text-sm">{searchQuery ? "No matching orders found" : showActions ? "No pending orders" : "No orders found"}</p>
        </div>
      );
    }
    return (
      <div className="space-y-3">
        {filtered.map((order: any) => (
          <div key={order.id} className="bg-muted/40 border border-border rounded-xl p-4 hover:border-primary/30 transition-colors">
            <div className="flex flex-col lg:flex-row lg:items-center gap-4">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground tabular-nums">
                  <span><strong className="text-foreground/80 font-medium">Amount:</strong> {formatAmount(order.amount, order.symbol)}</span>
                  <span><strong className="text-foreground/80 font-medium">Price:</strong> {formatPrice(order.price)}</span>
                  <span><strong className="text-foreground/80 font-medium">ID:</strong> #{order.id}</span>
                  <span><strong className="text-foreground/80 font-medium">Time:</strong> {formatDate((order as any).created_at || order.createdAt)}</span>
                </div>
                {order.userDetails && (
              ...
              {showActions && ["pending_approval", "pending"].includes(order.status) && (
                <div className="flex gap-2 flex-shrink-0">
                  <Button size="sm" onClick={() => approveMutation.mutate(order.id)} disabled={approveMutation.isPending}
                    className="bg-success hover:bg-success/90 text-success-foreground text-xs h-8 px-3">
    ...
    return (
      <AdminLayout>
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="bg-card rounded-xl border border-border p-5 animate-pulse h-72" />
        </div>
      </AdminLayout>
    );
  }
        ...
        {stats && (
          <>
            ...
            {(unreadSupportChats.length > 0 || stats.support.urgent > 0 || stats.financial.pendingDeposits > 0 || stats.financial.pendingWithdrawals > 0 || stats.trading.pendingTrades > 0) && (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                {unreadSupportChats.length > 0 && (
                  <button onClick={() => setLocation('/admin/support')} className="flex items-center gap-3 bg-primary/10 border border-primary/20 rounded-xl px-4 py-3 hover:bg-primary/15 transition-colors text-left">
                    <MessageSquare size={18} className="text-primary flex-shrink-0" />
                    <div className="min-w-0">
                      <p className="text-xs font-semibold text-primary">{unreadSupportChats.length} Unread Support Chat{unreadSupportChats.length === 1 ? '' : 's'}</p>
                      <p className="text-[10px] text-primary/70 truncate">{unreadSupportPreview}{unreadSupportChats.length > 2 ? ', …' : ''}</p>
                    </div>
                  </button>
                )}
                {stats.support.urgent > 0 && (
                  <a href="/admin/support" className="flex items-center gap-3 bg-danger/10 border border-danger/20 rounded-xl px-4 py-3 hover:bg-danger/15 transition-colors">
                    <AlertTriangle size={18} className="text-danger flex-shrink-0" />
                    <div>
                      <p className="text-xs font-semibold text-danger">{stats.support.urgent} Urgent Tickets</p>
                      <p className="text-[10px] text-danger/60">Requires immediate attention</p>
                    </div>
                  </a>
                )}
                {stats.financial.pendingDeposits > 0 && (
                  <button onClick={() => setLocation('/admin/wallets')} className="flex items-center gap-3 bg-warning/10 border border-warning/20 rounded-xl px-4 py-3 hover:bg-warning/15 transition-colors text-left">
                    <Clock size={18} className="text-warning flex-shrink-0" />
                    <div>
                      <p className="text-xs font-semibold text-warning">{stats.financial.pendingDeposits} Pending Deposits</p>
                      <p className="text-[10px] text-warning/60">Click to review</p>
                    </div>
                  </button>
                )}
                {stats.financial.pendingWithdrawals > 0 && (
                  <button onClick={() => setLocation('/admin/wallets')} className="flex items-center gap-3 bg-warning/10 border border-warning/20 rounded-xl px-4 py-3 hover:bg-warning/15 transition-colors text-left">
                    <Clock size={18} className="text-warning flex-shrink-0" />
                    <div>
                      <p className="text-xs font-semibold text-warning">{stats.financial.pendingWithdrawals} Pending Withdrawals</p>
                      <p className="text-[10px] text-warning/60">Click to review</p>
                    </div>
                  </button>
                )}
                {stats.trading.pendingTrades > 0 && (
                  <button onClick={() => setActiveTab('pending-orders')}
                    className="flex items-center gap-3 bg-info/10 border border-info/20 rounded-xl px-4 py-3 hover:bg-info/15 transition-colors text-left">
                    <BarChart3 size={18} className="text-info flex-shrink-0" />
                    <div>
                      <p className="text-xs font-semibold text-info">{stats.trading.pendingTrades} Pending Trades</p>
                      <p className="text-[10px] text-info/60">Click to review</p>
                    </div>
                  </button>
                )}
              </div>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-3 lg:grid-cols-6 gap-1.5 sm:gap-2">
              ...
              >
                <div className="relative w-7 h-7 sm:w-9 sm:h-9 bg-success/10 rounded-lg flex items-center justify-center text-success">
                ...
                <span className="text-[10px] sm:text-[11px] font-medium text-muted-foreground group-hover:text-foreground transition-colors text-center leading-tight">Deposit Requests</span>
              </button>
              ...
              <button
                ...
                className="group relative flex flex-col items-center gap-1 sm:gap-1.5 bg-card border border-border rounded-xl p-2 sm:p-3 hover:border-primary/40 transition-colors"
              >
                <div className="relative w-7 h-7 sm:w-9 sm:h-9 bg-info/10 rounded-lg flex items-center justify-center text-info">
                  <DollarSign className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                ...
                <span className="text-[10px] sm:text-[11px] font-medium text-muted-foreground group-hover:text-foreground transition-colors text-center leading-tight">Withdraw Requests</span>
              </button>
              ...
              <button
                ...
                className="group relative flex flex-col items-center gap-1 sm:gap-1.5 bg-card border border-border rounded-xl p-2 sm:p-3 hover:border-primary/40 transition-colors"
              >
                <div className="relative w-7 h-7 sm:w-9 sm:h-9 bg-muted rounded-lg flex items-center justify-center text-muted-foreground">
                  <Clock className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                </div>
                <span className="text-[10px] sm:text-[11px] font-medium text-muted-foreground group-hover:text-foreground transition-colors text-center leading-tight">Deposit History</span>
              </button>
              ...
              >
                ...
                <span className="text-[10px] sm:text-[11px] font-medium text-muted-foreground group-hover:text-foreground transition-colors text-center leading-tight">Withdraw History</span>
              </button>
              ...
              <button
                ...
                className="group relative flex flex-col items-center gap-1 sm:gap-1.5 bg-card border border-border rounded-xl p-2 sm:p-3 hover:border-primary/40 transition-colors"
              >
                <div className="relative w-7 h-7 sm:w-9 sm:h-9 bg-primary/10 rounded-lg flex items-center justify-center text-primary">
                ...
                <span className="text-[10px] sm:text-[11px] font-medium text-muted-foreground group-hover:text-foreground transition-colors text-center leading-tight">KYC Mgmt</span>
              </button>
              ...
              <button
                ...
                className="group relative flex flex-col items-center gap-1 sm:gap-1.5 bg-card border border-border rounded-xl p-2 sm:p-3 hover:border-primary/40 transition-colors"
              >
                <div className="relative w-7 h-7 sm:w-9 sm:h-9 bg-warning/10 rounded-lg flex items-center justify-center text-warning">
                ...
                <span className="text-[10px] sm:text-[11px] font-medium text-muted-foreground group-hover:text-foreground transition-colors text-center leading-tight">Loan Mgmt</span>
              </button>
            </div>
                ...
                {['orders', 'pending-orders', 'activity'].map(tab => {
                  ...
                  return (
                    <TabsTrigger key={tab} value={tab}
                      className="relative rounded-lg data-[state=active]:bg-primary/10 data-[state=active]:text-primary text-muted-foreground text-xs px-3 py-1.5 capitalize">
                      {label}
                      {badge > 0 && (
                        <span className="ml-1.5 min-w-[18px] h-[16px] px-1 bg-danger text-danger-foreground text-[9px] font-bold rounded-full inline-flex items-center justify-center">
                          {badge > 99 ? '99+' : badge}
                        </span>
                      )}
                    </TabsTrigger>
                ...
                <div className="bg-card rounded-xl border border-border">
                  <div className="p-4 border-b border-border flex flex-col sm:flex-row sm:items-center gap-3">
                    <div className="flex-1">
                      ...
                      <p className="text-[11px] text-muted-foreground">{allOrders?.length || 0} total orders</p>
                    </div>
                ...
                <div className="bg-card rounded-xl border border-border">
                  <div className="p-4 border-b border-border flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-muted-foreground">{pendingOrders?.length || 0} orders waiting</p>
                    </div>
                  </div>
                  <div className="p-4">
                    {ordersLoading ? (
              ...
              <TabsContent value="activity" className="mt-4">
                <div className="bg-card rounded-xl border border-border">
                  <div className="p-4 border-b border-border">
                    ...
                    <p className="text-[11px] text-muted-foreground">Latest platform events across all modules</p>
                  </div>
                  <div className="divide-y divide-border">
                    {stats.recentActivity.length === 0 ? (
                      ...
                      stats.recentActivity.map((item, i) => (
                        <div key={i} className="flex items-center gap-3 px-4 py-3 hover:bg-muted/40 transition-colors cursor-pointer" onClick={() => handleActivityClick(item.type)}>
