...
export default function AdminTradingPairs() {
    ...
    try {
      ...
      const response = await fetch('/api/admin/trading-pairs', {
        ...
        body: JSON.stringify({
          ...
          customApiUrl: newPair.pairType === 'custom' ? newPair.customApiUrl : undefined,
        }),
      });
      if (response.ok) {
        const data = await response.json();
        ...
        setNewPair({ baseAsset: '', quoteAsset: 'USDT', pairType: 'both', minTradeAmount: '0.001', maxTradeAmount: '1000', tradingFeePercent: '0.10', customApiUrl: '' });
  ...
  const startEdit = (pair: TradingPair) => {
    ...
    setEditForm({
      ...
      custom_api_url: pair.custom_api_url || '',
    });
  };
    ...
    try {
      ...
      const response = await fetch(`/api/admin/trading-pairs/${editingId}`, {
        ...
        body: JSON.stringify({
          ...
          customApiUrl: editForm.custom_api_url || null,
        }),
      });
      if (response.ok) {
        const updated = await response.json();
