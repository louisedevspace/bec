...
function Router() {
  ...
  // Seeded from the cached profile so a refresh on /admin/support doesn't
  // bounce a support agent through the customer app first.
    ...
    try {
      ...
      if (currentUser) {
        try {
          ...
          const isSupport = !error && freshProfile?.role === 'support';
          ...
          // Support agents are staff accounts — send them straight to the
          // support inbox instead of the customer-facing app.
          if (isSupport && location !== '/admin/support') {
            setLoading(false);
    ...
    <MainLayout>
      <Suspense fallback={<LoadingScreen />}>
        <Switch>
          ...
          {/* Support inbox — admins and support agents; agent management stays admin-only */}
          <Route path="/admin/support" component={isAuthenticated && (isAdmin || isSupportAgent) ? AdminSupport : adminRedirect} />
          <Route path="/admin/support-agents" component={isAuthenticated && isAdmin ? AdminSupportAgents : adminRedirect} />
